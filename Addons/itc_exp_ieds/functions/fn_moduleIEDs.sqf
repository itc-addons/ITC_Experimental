...
switch (_IEDType) do {
	case "CWIED": {
			...
			[{
				(_this # 0) params ["_ied","_friendlysides","_deleteThese","_collapsing","_logic","_triggers"];
			...
			},0.5,[_ied,_friendlysides,_deleteThese,_collapsing,_logic,_triggers]]  call CBA_fnc_addPerFrameHandler;
		};
	case "RCIED": {
		...
		[{
			(_this # 0) params ["_ied","_friendlysides","_collapsing","_logic","_switch","_triggers"];
			if (((({((_x distance _ied) < 5) && !((side _x) in _friendlySides) && !(unitIsUAV vehicle _x)} count allUnits) > 0) && 
			...
			((count ([_triggers,{(alive _x) && (([objNull, "VIEW"] checkVisibility [eyepos _x, [getPosASL _ied # 0,getPosASL _ied # 1,(getPosASL _ied # 2)+1]]) > 0)}] call BIS_fnc_conditionalSelect)) > 0) &&
			 ...
			 !({(_x distance _ied) < (_x getVariable "H_ECM")} count allUnits > 0))) then {
		...
		},0.5,[_ied,_friendlysides,_collapsing,_logic,_switch,_triggers]]  call CBA_fnc_addPerFrameHandler;
	};
};